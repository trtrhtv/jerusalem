# Photo attribution · קרדיט תמונות

All images in this directory are **public domain** ("no known restrictions on
publication") and are hosted here as source anchors for the timeline map. Each
entry links to the verified catalog record at the holding institution. File
names match viewpoint IDs in `data/viewpoints.json`.

| File | Title | Creator / Collection | Date | Repro. no. | Record |
| --- | --- | --- | --- | --- | --- |
...
Rule for adding photos: public-domain items only, one row here per file, and a
`localImage` field on the matching viewpoint in `data/viewpoints.json`
(`npm run validate` enforces both directions).
